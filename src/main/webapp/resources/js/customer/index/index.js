...
function validateUserForm(){    
    $('#changePassWordForm').validate({
        ...
        submitHandler : function(form) {
        	 ...
        	 $.ajax({
        	        type : "POST",
        	        url : sys.rootPath + "/changePassWord.action",
        	        data : data,
        	        dataType : "json",
        	        success : function(resultdata) {
        	            if (resultdata.success) {
...
function formatCellImg(cellvalue,options,rowObject){
	if(cellvalue!="")
		return "<a href="+cellvalue+" target='_Blank'><img alt='' src="+cellvalue+" style='height: 45px;width: auto;max-width:70px;'></a>";
	return cellvalue;
}
//验证管理员账号
...
function startTimer(){
	...
	timer=setTimeout(function(){
		...
		location.href=sys.rootPath+"/login.jsp";
	},1800000);
}
document.onkeydown=document.onmousedown=startTimer;
startTimer();
//格式化列表标题图片
function formaterImg(cellvalue, options, rowdata) {
    if(rowdata.img){
        return '<a target="_Blank" href="'+rowdata.img+'" >  <img src="' + rowdata.img + '"  style="width:30px;height:20px;"/></a>';
    }
    return "无图片";
}