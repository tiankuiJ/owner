...
<script type="text/javascript">
    $("#jsonmap").jqGrid({
        url: sys.rootPath + '/opInfo/list.action?type=表决',
        datatype: "json",
        mtype: "GET",
    ...
    function recoderUI(id) {
        localStorage.setItem("bjids",null);
        localStorage.setItem("oid", id);
        localStorage.setItem("orgId", $("#jsonmap").jqGrid('getCell',id,'orgId'));
        localStorage.setItem("op", "表决");
        localStorage.setItem("from", "biaojue");
        loadPage("/regRecord/listUI.action","表决参加记录");
    }


    function editUI(id) {
        localStorage.setItem("opType","表决");
        loadPage("/opInfo/formUI.action?id="+id,"表决详情");
    }

    function add() {
        localStorage.setItem("opType","表决");
        loadPage("/opInfo/formUI.action","添加表决");
    }
    function reloadGrid() {
