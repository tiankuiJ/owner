...
<script type="text/javascript">
    $("#jsonmap").jqGrid({
        url: sys.rootPath + '/opInfo/list.action?type=意见',
        datatype: "json",
        mtype: "GET",
    ...
    function recoderUI(id) {
        ...
        localStorage.setItem("from", "yijian");
        localStorage.setItem("op", "意见");
        loadPage("/regRecord/listUI.action","意见参加记录");
    }


    function editUI(id) {
        localStorage.setItem("opType","意见");
        loadPage("/opInfo/formUI.action?id="+id,"意见详情");
    }

    function add() {
        localStorage.setItem("opType","意见");
        loadPage("/opInfo/formUI.action","添加意见");
    }
    function reloadGrid() {
